...
public interface DocumentRepository extends JpaRepository<DocumentEntity, Long> {
    ...
    @Query("select d from DocumentEntity d inner join BaseDocumentEntity b on d.id = b.id " +
    ...
    @Query("select d from DocumentEntity d inner join BaseDocumentEntity b on d.id = b.id " +
            "inner join CategoryEntity c on c.id = b.category.id where c.name like ?1 and b.name like %?2%")
